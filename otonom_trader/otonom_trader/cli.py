...
@app.command()
def experiments_grid_search(
    experiment_name: str = typer.Argument(..., help="Experiment name"),
    strategy: str = typer.Option("baseline_v1", help="Strategy name (YAML file in strategies/)"),
    grid: str = typer.Option("baseline_grid_v1", help="Parameter grid name (YAML file in grids/)"),
    db: str = typer.Option("trader.db", help="Database path"),
):
    """
    Run grid search experiment for parameter optimization.

    Example:
        otonom-trader experiments-grid-search my_experiment --strategy baseline_v1 --grid baseline_grid_v1
    """
    from .experiments import run_grid_search
    from pathlib import Path

    strategy_path = Path(f"strategies/{strategy}.yaml")
    grid_path = Path(f"grids/{grid}.yaml")

    if not strategy_path.exists():
        typer.echo(f"✗ Strategy file not found: {strategy_path}", err=True)
        raise typer.Exit(code=1)

    if not grid_path.exists():
        typer.echo(f"✗ Parameter grid file not found: {grid_path}", err=True)
        raise typer.Exit(code=1)

    typer.echo(f"Starting grid search experiment: {experiment_name}")
    typer.echo(f"Strategy: {strategy_path}")
    ...
    try:
        ...
        with get_session(db) as session:
            ...
            typer.echo("")
            typer.echo("=" * 80)
            typer.echo(f"EXPERIMENT COMPLETE: {experiment.name}")
            typer.echo("=" * 80)
            typer.echo(f"Total runs: {len(experiment.runs)}")

            # Find best run by test sharpe
            successful_runs = [r for r in experiment.runs if r.status == "done" and r.test_sharpe is not None]
            ...
            if successful_runs:
                ...
                typer.echo("")
                typer.echo("Best run:")
                typer.echo(f"  Run index: {best_run.run_index}")
                typer.echo(f"  Parameters: {best_run.param_values_json}")
                typer.echo(f"  Train Sharpe: {best_run.train_sharpe:.2f}")
                typer.echo(f"  Test Sharpe: {best_run.test_sharpe:.2f}")
                typer.echo(f"  Test CAGR: {best_run.test_cagr:.2f}%")
                typer.echo(f"  Test Max DD: {best_run.test_max_dd:.2f}%")

                # Check for overfitting
                if best_run.train_sharpe > 0 and best_run.test_sharpe > 0:
                    ratio = best_run.train_sharpe / best_run.test_sharpe
                    if ratio > 1.5:
                        typer.echo(f"  ⚠ WARNING: Possible overfitting (train/test ratio: {ratio:.2f})")
                    else:
                        typer.echo(f"  ✓ Good generalization (train/test ratio: {ratio:.2f})")

            typer.echo("")
            typer.echo(f"Results saved to database: {db}")
...
def experiments_list(
    ...
    Example:
        otonom-trader experiments-list
    """
    ...
    try:
        with get_session(db) as session:
            ...
            typer.echo("=" * 100)
            typer.echo(f"{'ID':<6} {'Name':<30} {'Strategy':<20} {'Runs':<8} {'Created':<20}")
            typer.echo("-" * 100)

            for exp in experiments:
                created = exp.created_at.strftime("%Y-%m-%d %H:%M:%S")
                typer.echo(f"{exp.id:<6} {exp.name:<30} {exp.base_strategy_name:<20} {len(exp.runs):<8} {created:<20}")
...
def experiments_show(
    ...
    Example:
        otonom-trader experiments-show 1 --top-n 10
    """
    from .data import Experiment
    ...
    try:
        with get_session(db) as session:
            ...
            typer.echo("=" * 100)
            typer.echo(f"Strategy: {experiment.base_strategy_name}")
            typer.echo(f"Created: {experiment.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
            typer.echo(f"Total runs: {len(experiment.runs)}")

            # Get successful runs with test results
            successful_runs = [
            ...
            if not successful_runs:
                typer.echo("\nNo successful runs with test results.")
                return

            # Sort by test sharpe
            successful_runs.sort(key=lambda r: r.test_sharpe, reverse=True)

            typer.echo("")
            typer.echo(f"Top {top_n} runs by test Sharpe:")
            typer.echo("")
            typer.echo(f"{'Rank':<6} {'Run':<6} {'Train Sharpe':<14} {'Test Sharpe':<13} {'Test CAGR':<12} {'Test DD':<10}")
            typer.echo("-" * 100)

            for rank, run in enumerate(successful_runs[:top_n], start=1):
                typer.echo(
                    f"{rank:<6} {run.run_index:<6} "
                    f"{run.train_sharpe:>13.2f} {run.test_sharpe:>12.2f} "
                    f"{run.test_cagr:>11.2f}% {run.test_max_dd:>9.2f}%"
                )
            ...
            if successful_runs:
                ...
                typer.echo(best_run.param_values_json)

    except Exception as e:
        typer.echo(f"✗ Error showing experiment: {e}", err=True)
        raise typer.Exit(code=1)
